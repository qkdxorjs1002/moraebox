...
fn find_helper() -> Option<PathBuf> {
    if let Some(configured) = env::var_os("FASTMVM_HELPER_PATH").map(PathBuf::from)
        && configured.is_file()
    {
        return Some(configured);
    }
    let sibling = env::current_exe().ok()?.with_file_name(if cfg!(windows) {
        "fastmvm-vmm-helper.exe"
    } else {
        "fastmvm-vmm-helper"
    });

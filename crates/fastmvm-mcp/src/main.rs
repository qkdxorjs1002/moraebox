...
use clap::Parser;
use fastmvm_core::{OutputChunk, RunSpec, SessionId, Signal, TimeoutPolicy};
use fastmvm_runtime::{Backend, LibkrunBackend, LibkrunConfig, ProcessBackend};
use fastmvm_sdk::{ExecutionResult, IoRequest, IoResult, SandboxSdk};
...
#[derive(Debug, Parser)]
#[command(name = "fastmvm-mcp", about = "stdio MCP server for fastmvm")]
struct Args {
...
fn create_sdk(args: Args) -> Result<SandboxSdk, String> {
    let backend: Arc<dyn Backend> = match args.backend.as_str() {
        ...
        "libkrun" => {
            let helper = args
                .helper
                .ok_or("libkrun backend requires --helper or FASTMVM_HELPER_PATH")?;
            let library = args
                .libkrun
                .ok_or("libkrun backend requires --libkrun or FASTMVM_LIBKRUN_PATH")?;
            let root = args
                .rootfs
                .ok_or("libkrun backend requires --rootfs or FASTMVM_ROOTFS")?;
    ...
    match method {
        "initialize" => success(
            ...
            json!({
                ...
                "serverInfo": { "name": "fastmvm", "version": env!("CARGO_PKG_VERSION") },
